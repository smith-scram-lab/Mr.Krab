...
Adapted from code written by Julia Yu, Smith College '24, to supply enough power to move servos to their positions effectively

Written by: Rachel Reinking, Smith College '25
"""
...
def move(servoList, threshold: int) -> None:

    """
    Sets goal positions of servos until all servos have been moved to the correct position

    args:
        servoList (list of Servo): list of Servos
        threshold (int): threshold interval for servo position
    """
    goal_positions_list = []
    # Write goal position. This is what causes the servos to actually move
    ...
    while move:
        ...
        for a in goal_positions_dict.keys():
            ...
            if (goal_positions_dict[a] - threshold <= servo_pos_dict[a] <= goal_positions_dict[a] + threshold):
            ...
            for servo in range(len(servoList)):
                if not move_verdict[servo]: # this is here in case the servo stops when it hasn't reached the goal, this makes it keep moving by resending the goal
                    ...
                    while not is_set_goal:
                        is_set_goal = servoList[servo].set_goal_position(GOAL_DESTINATION)
                    #print("Servo ID #" + str(servoList[servo].id) + " has NOT reached goal position")
                #else:
                    #print("Servo ID #" + str(servoList[servo].id) + " has reached goal position")

def main() -> None:
    # Instances of the servos, grouped by position on each leg
    # Servos nearest to the base of the hexapod on each leg
    servoListBase = [Servo(DXL_ID1), 
                     Servo(DXL_ID4),
                     Servo(DXL_ID7),
                     Servo(DXL_ID10), 
                     Servo(DXL_ID13), 
    ...
    # Servos on the middle of each leg
    servoListMiddle = [Servo(DXL_ID2),
                       Servo(DXL_ID5), 
                       Servo(DXL_ID8),
                       Servo(DXL_ID11), 
                       Servo(DXL_ID14), 
    ...
    # Servos on the end of each leg
    servoListEnd = [Servo(DXL_ID3),
                    Servo(DXL_ID6), 
                    Servo(DXL_ID9), 
                    Servo(DXL_ID12),
                    Servo(DXL_ID15),
    ...
    # this order ensures that all servos are supplied enough power to be able to move to the right positions
    servoList = [servoListMiddle, servoListEnd, servoListBase]

    for servoGroup in servoList:
        # Enable Dynamixel Torque
        print("Enabling torque")
        for servo in servoGroup:
        ...
        # Move the group of servos
        move(servoGroup, DXL_MOVING_STATUS_THRESHOLD)        
 
    # Move all servos to return to positions that may have shifted when moving other servo groups
    move(servoListBase + servoListMiddle + servoListEnd, DXL_MOVING_STATUS_THRESHOLD)

    # Disable Dynamixel Torque
    for servoGroup in servoList:
        for servo in servoGroup:
            is_set_torque = servo.set_torque_value(TORQUE_DISABLE) # disable torque
            while not is_set_torque: # if failed tp disable torque, force it to keep running on the same servo until it works
    ...
    for servoGroup in servoList:
        for servo in servoGroup:
            get_current = servo.get_current_position() # get current position value
            while get_current == False: # if failed to get goal position, force it to keep running on the same servo until it works
                get_current = servo.get_current_position()
